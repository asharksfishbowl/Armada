...
| **D4** | agent-runtime | R17/R18's liveness check verifies that a pinned `binding_tag` is present and `promoted`. A registered-but-unmaterialized binding passes that check and the Run then blocks behind a multi-gigabyte download. | Extend the liveness check to `materialized`, failing Run start per Requirement 9. | **P7** |
...
**Depends on:** P0.
`db/migrations/003_training.sql` — which creates `datasets`, `training_runs`, `adapters`, `evaluations`, and `model_bindings` together, of which only `model_bindings` is exercised here — shortlist validation, R4a base binding registration, R4b reconciliation, `GET /models/bindings`, the `materialized` field of Requirement 7, the capacity enforcement of Requirements 15 and 16, and the backend discriminator of Requirement 19.
...
**Depends on:** P0.
`db/migrations/005_runs_events.sql`, the plugin registry and the five plugin interfaces, the single-port HTTP and WebSocket listener, transactional gapless `seq`, ordered replay on subscribe, and credential redaction.
...
### P4 — Agent definition
**Depends on:** P1, P2, P3.
`db/migrations/004_agents.sql`, the closed schema, error-accumulating validation, reference resolution, the resolver and `resolved_snapshot`, versioning, `refresh-bindings`, the file loader, and both shipped examples.
