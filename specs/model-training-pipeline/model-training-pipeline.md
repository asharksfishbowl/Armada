...
8. Ingestion fetches each Source: `git` clones at depth 1 to a temp directory; `web` crawls same-origin links to a depth of 2; `directory` reads a path under `ARMADA_INGEST_ROOT`; `upload` reads a previously uploaded blob.
8a. `ARMADA_INGEST_ROOT` (default `/var/lib/armada/ingest`) is bind-mounted read-only into `armada-forge` at the same path it occupies on the host, so a path the operator types is the path the forge opens. It is **not** the daemon's `ARMADA_WORKSPACE_ROOT`: that root exists so the daemon can hand Docker a host path when provisioning a sandbox, which is a runtime concern. Sharing one mount for both would couple sandbox provisioning to corpus ingestion across cross-service boundary 1.
8b. `POST /corpora/{corpus_id}/sources` with `type: directory` **rejects** a `location` that is not resolvable and readable inside `armada-forge`, returning HTTP 400 naming the path, `ARMADA_INGEST_ROOT`, and the requirement that the path be bind-mounted into the forge service. A `location` that escapes `ARMADA_INGEST_ROOT` — by `..`, a symlink, or an absolute path outside it — is rejected on the same terms.

The check happens at registration rather than at ingest because the alternative is silent: an unreadable path matches no files, and an ingestion job over it would otherwise complete `succeeded` with `chunks_added: 0`. An operator would see a green ingest and an empty Corpus with nothing anywhere explaining why.
8c. An ingestion job in which **every** Source yields zero extractable files terminates `failed`, not `succeeded`, naming each Source and its match count. Zero files from a Source the operator explicitly registered is a fault, not an empty result — and reporting it as success is how a misconfiguration survives to be discovered much later.
...
24a. **Smoke mode** (no CUDA device — the default on the target CPU-only host). It rejects any `TrainingConfig` whose `base_model_id` does not have `smoke_test: true`, returning a validation error naming the constraint. It caps `max_steps` at 20, `max_samples` at 200, `max_seq_len` at 1024, and `batch_size` at 1, regardless of requested values, and records both the requested and the clamped value for each on the training run. All four caps are required: step cost is roughly linear in the product of `batch_size` and `max_seq_len`, so capping only `max_steps` and `max_samples` bounds nothing — a caller passing `batch_size: 8, max_seq_len: 4096` satisfies every stated cap while turning a fifteen-minute smoke run into many hours on the CPU-only host the cap exists to protect. It records `run_kind: smoke`. A smoke run proves the pipeline executes end to end; it is not expected to produce a model worth serving, and is never promotable (Requirement 37).
...
34. The evaluation gate runs in one of two modes, selected by `mode` in `config/eval.yaml` (one of `mechanical`, `judge`; default `mechanical`). Both modes generate a completion per held-out sample from the candidate Adapter and from the unmodified BaseModel **in-process inside `armada-forge`**, loading base weights plus the unmerged adapter through transformers and PEFT. **`armada-models` is not involved in generation or in scoring.** and both compute `tool_call_validity` — the fraction of tool calls emitted across those completions that parse and validate against their declared schema. Generation is local in both modes and costs nothing.
34e. The in-process execution model of Requirement 34 is not an optimization; it is the only buildable path, for two independent reasons. First, Requirement 30 forbids registering an unpromoted Adapter with `armada-models` and Requirement 31 produces the GGUF only on promotion, so at gate time the candidate has no tag to address — generating through `armada-models` is a closed loop, since the gate must decide promotion and promotion is what would make the candidate addressable. Second, teacher-forced scoring of a supplied continuation requires per-token logprobs over supplied tokens, which the `armada-models` OpenAI-compatible surface does not expose by any route, so `held_out_perplexity` is unobtainable through it. An eval-scoped registration exception was rejected as the alternative because it makes an unpromoted Adapter briefly servable to the whole daemon — the exact state Requirement 30 exists to forbid. `armada-forge` already carries torch, transformers, and PEFT for `LocalTrainingBackend`, so this adds no dependency.
...
22. When `config/teacher.yaml` has `enabled: false` and a `POST /datasets` names a `corpus_id`, the request is rejected with HTTP 400 naming the two teacher-free sources. No outbound request is made, so a default installation with no credentials configured never attempts to reach an external endpoint.
23. When `config/eval.yaml` sets `mode: judge` while `config/teacher.yaml` has `enabled: false`, `armada-forge` fails startup naming both settings rather than deferring the failure to the first promotion attempt.
24. When a supplied JSONL line is missing `instruction` or `response`, or has either as an empty string, `POST /datasets/supplied` rejects the whole upload listing the offending line numbers and stores nothing.
25. When a smoke Adapter is created, the `evaluations` table receives no row for it — the rejection at Requirement 33b precedes evaluation entirely, so `judge_errors`, `samples_evaluated`, and the score sets are absent rather than zero.
26. When `LocalTrainingBackend` detects no CUDA device and the request names a `base_model_id` without `smoke_test: true`, the run is rejected naming both the model and the absent GPU. It is never silently downgraded to the smoke model, because a caller would otherwise believe it had trained the model it asked for.
27. When a CUDA device is added to the host and `armada-forge` restarts, `LocalTrainingBackend` switches to quality mode with no configuration change; previously recorded smoke runs keep `run_kind: smoke` and remain unpromotable.
28. When the gate runs in mechanical mode, `judge_errors` is 0 and `task_success_rate` is null in both score sets; promotion is decided on `held_out_perplexity` and `tool_call_validity` alone.
29. When `armada-models` already serves a tag matching a base ModelBinding at startup, that binding is not re-registered and startup proceeds; startup registration is idempotent.
30. When a BaseModel entry is removed from `config/base-models.yaml` while an Agent is still bound to its base tag, the binding is set to `status: retired` and `GET /models/bindings` reports it; Run start against a retired binding fails with the runtime's missing-binding error rather than silently serving a stale model.
31. When two Corpora are created whose `name` values differ only by case, the second is rejected by the `^[a-z0-9-]+$` constraint, so `corpus_name` in a ModelBinding tag is never ambiguous.
...
- [ ] Registering a `directory` Source whose `location` forge cannot read is refused with HTTP 400 naming the path and `ARMADA_INGEST_ROOT`, and no `sources` row is written.
- [ ] Registering a `directory` Source whose `location` escapes `ARMADA_INGEST_ROOT` via `..` or a symlink is refused on the same terms.
- [ ] An ingestion job in which every Source yields zero extractable files terminates `failed`, naming each Source and its match count — never `succeeded` with `chunks_added: 0`.
...
- [ ] **Judge mode only.** An Adapter whose `task_success_rate` is below its BaseModel's is set to `rejected`, and both score sets are readable from the `evaluations` table. In mechanical mode `task_success_rate` is null and the comparison rests on the remaining scored metrics.
- [ ] `POST /training/runs` against a dataset with no eval split is rejected with an error naming the missing split **when the run could produce a promotable Adapter**; a smoke run against the same split-less dataset is accepted, per Requirement 33a.
...
- [ ] **Judge mode only.** The evaluation gate issues at most `max_eval_samples` judge calls, and re-running the gate for the same `adapter_id` scores the identical sample subset.
- [ ] **Judge mode only.** With the teacher endpoint unreachable, the evaluation gate leaves the Adapter at `pending_eval` and does not set it to `rejected`. In mechanical mode the teacher is never contacted and its reachability is irrelevant.
...
- [ ] A trajectory-only dataset is rejected by `POST /datasets/{dataset_id}/split` but is accepted by `POST /training/runs` with `backend: local` in smoke mode, because the split exists solely to gate promotion and a smoke run is never promotable.
- [ ] The evaluation gate completes with `armada-models` stopped, proving neither generation nor scoring depends on it.
- [ ] A smoke `TrainingConfig` requesting `batch_size: 8` and `max_seq_len: 4096` is clamped to 1 and 1024, both the requested and clamped values are recorded, and the run completes in the same order of magnitude as one requesting the defaults.
