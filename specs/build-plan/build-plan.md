...
4. **Materialization is a new operation owned by `armada-forge`**, exposed as `POST /models/bindings/{tag}/materialize`. Ownership follows `platform-overview.md` cross-service boundary 2: the forge registers ModelBindings with `armada-models`, so it materializes them. It is not one of the additive endpoints the design spec's dependency rulings cover — those surfaced existing data behind existing routes, whereas this creates new state and a long-running job.
5. Materialization ships in **Phase 2, ahead of its user interface, deliberately.** Requirement 9's fail-fast would otherwise name a state the operator has no way to exit until Phase 9. An endpoint operable by `curl` before it has a UI is the lesser problem.
6. Every other entry in `config/base-models.yaml` is **lazily materialized**. Model & Training Pipeline R4a registers a base ModelBinding per shortlist entry at startup; registration writes the `model_bindings` row and sets `materialized: false`. The operator triggers materialization explicitly.
7. `model_bindings` carries a `materialized` boolean and a `materialization_status`. `GET /models/bindings` returns it. Registration and materialization are separate acts, and a binding may be `promoted` and unmaterialized simultaneously.
8. A first `docker compose up` on a clean host downloads container images and nothing else. From that state the entire zero-cost path runs offline: ingest a Corpus, upload a JSONL, run a smoke training job, define an Agent against `armada/qwen3-0.6b-base`, and run it with retrieval and sandboxed tools.
9. `POST /api/runs` **fails fast** when the pinned `binding_tag` resolves to an unmaterialized binding, naming the tag and the required action. A Run must never block silently behind a multi-gigabyte download. This is the dashboard-visible half of upstream defect D4.

### Materialization surface

10. Materialization progress renders in the **inline-row treatment**, not the training stage-rail treatment. The two are distinct locked patterns in `specs/dashboard/design-dashboard.md`: the stage rail is for a multi-stage pipeline on a detail surface opened deliberately, and a one-segment rail is dead chrome. Materialization is single-stage and fired from a list row the operator navigates away from, which is the ingestion pattern exactly.
11. The ModelsPage cell that renders `Download · {n} GB` becomes a 2px progress line with `downloading · {done} / {total} GB` in place, with the live shimmer. On completion it becomes the achromatic `local` flag chip and all motion stops.
12. The `last update {duration} ago` staleness escalation applies unchanged and is **more necessary here than for training**, because Requirement 25 makes the channel lossy: a stalled download and a dropped message are indistinguishable from the client without a relative timestamp.
13. A failed materialization renders **amber, not red**: the cell reverts to `Download` with `retry · last attempt failed`. A failed pull is a recoverable environment condition, not a system fault, and `--status-fault` stays reserved.
14. If `armada-forge` serializes concurrent materializations, waiting rows render `--status-neutral` `○ QUEUED`, reusing the delegation queue-row treatment unchanged. If it does not serialize, rows are independent and nothing changes. Either way no new pattern is introduced.

### Host capacity and resource ceilings

15. `config/base-models.yaml`'s `min_ram_gb` is enforced rather than decorative. Materialization is refused when the host's available memory is below the entry's `min_ram_gb`, naming both values.
16. `config/base-models.yaml` gains a `min_disk_gb` sibling to `min_ram_gb`, enforced identically at materialization. Disk is the binding constraint for any expert-streaming backend and for large quantized weights generally.
17. `docker-compose.yml` sets explicit memory limits on `armada-forge` and `armada-models`. A CPU training run and a concurrent ingestion embed contend on the same host, and the unbounded failure mode is an OOM kill with no attribution.
18. `docker-compose.yml` sets `OLLAMA_MAX_LOADED_MODELS` and `OLLAMA_NUM_PARALLEL` to explicit values consistent with `config/models.yaml`'s `max_concurrent_per_tag` and `max_concurrent_total`. Without them the daemon's scheduler limits are fiction, because Ollama will serialize or evict beneath them.

### Model registry backend discriminator

19. The model registry lands with a backend discriminator in Phase 2 even though only Ollama is implemented, because retrofitting a discriminator into a populated `model_bindings` table, a shipped config schema, and a live `GET /models/bindings` contract is materially more work than defaulting a field on an empty one. Four changes:
...
20. Requirement 19.4 is what keeps this change small. Model & Training Pipeline edge 7 already rejects `POST /training/runs` against a `trainable: false` model, so forcing the flag reuses an existing guard and requires no change to the promotion path, the evaluation gate, or the ModelBinding tag scheme.

### Schema

20a. **Every migration is applied in Phase 0, in filename order, by `db/apply-migrations.sh`.** Schema is a foundation concern; later phases build *logic* against tables that already exist and own no migration of their own. Where a phase description below names a migration, it names the file that defines the tables that phase exercises, not a file that phase creates.
20b. Requirement 20a is not a convenience. `005_runs_events.sql` declares `agent_version_id uuid NOT NULL REFERENCES agent_versions(agent_version_id)` — an enforced foreign key into tables created by `004_agents.sql`. Phase 3 builds the event log and Phase 4 builds agent definition, so a per-phase migration order would require Phase 3 to create `runs` before Phase 4 creates the table it references. Applying the full set in filename order at Phase 0 dissolves that conflict rather than working around it.
20c. Phase 3's event-log exit condition is therefore verified against a **synthetic `runs` row** inserted against the Phase 0 schema, with a synthetic `agent_versions` parent to satisfy the foreign key. Phase 4 replaces the synthetic parent with real Agent versions and owns no schema change.

### Sequencing constraints

21. **Split by consumer, not by table.** An endpoint ships in the same phase as its consumer, and a consumer ships in the same phase as its endpoint. Neither ships ahead of the other. A shared migration does not license shipping an endpoint early: an endpoint verified only against an empty table is a shape test rather than a working feature.
22. Requirement 21 places `GET /training/runs`, `GET /adapters`, `GET /datasets`, and `GET /api/config/capabilities` in Phase 11 rather than Phase 9. Their producers are Phase 11, and so are all of their consumers: `teacher_enabled` drives the build-dataset modal on `TrainingPage`, `eval_mode` drives whether the adapter drawer renders `task_success_rate` or `held_out_perplexity`, and `local_backend_mode` drives the training launcher. Nothing in Phase 9 reads capabilities — the health strip reads `GET /api/health`, which exists from Phase 3.
23. Requirement 21 does **not** apply to a producer whose correctness is verifiable within its own phase by a test client rather than by a UI. The forge progress channel of Requirements 24–27 is built in Phase 1 and consumed in Phase 9, which is acceptable because Phase 1 can assert real progress messages against a real ingestion.

### The forge progress channel

24. `armada-forge` exposes a dashboard WebSocket channel carrying progress for long-running jobs. It is **created in Phase 1 for ingestion**, extended in Phase 2 for materialization, and extended in Phase 11 for training runs. Building it first on the simplest case and against three eventual consumers is why it is a shared primitive rather than a training-specific feature.
25. **The forge progress channel is not the daemon's `/ws` and must never be conflated with it.** Different service, different port, different semantics. Daemon events are gapless, append-only, and replay in full on subscribe (invariant 5, Agent Runtime R6). Forge progress messages are **lossy and non-replayable**: a client that misses one has missed it.
26. The channel's message envelope is defined by this spec because no feature spec owns it: `job_kind` (one of `ingestion`, `materialization`, `training`), `job_id`, `status`, `progress`, `total`, and `message` (nullable). A consumer keys on `job_kind` and `job_id`.
27. **REST is authoritative for state; the progress channel is authoritative only for increments.** On page load and on reconnect a consumer reads status from the REST resource and renders that; channel messages advance the progress value and never establish state. This is the same split Agent Runtime's event stream uses for runs, and it is stated once here for both.

### Sandbox host contract

28. The Docker socket mount into `armada-daemon` is stated as an explicit requirement rather than left implied by a Dockerfile. The daemon spawns sibling containers, which requires `/var/run/docker.sock` mounted into it, which is root-equivalent on the host. It violates no invariant — invariant 3 governs the sandbox boundary, and the platform scope is single-operator on a trusted network — but it is the highest-privilege decision in the deployment and must be visible.
29. Sandbox containers run non-root with dropped capabilities, no Docker socket, `/workspace` bind-mounted and `/armada` as tmpfs, and `network: none`. `egress_allowlist` is **not** available before Phase 14; see upstream defect D7.
30. Orphaned sandbox containers are labelled `armada.run_id` and swept on daemon startup by filtering on that label, satisfying Agent Runtime edge 13.

### Evaluation gate execution model

31. The evaluation gate runs **in-process inside `armada-forge`** using transformers and PEFT against the base weights plus the unmerged adapter. `armada-models` is not involved in either generation or scoring. This is the correction for upstream defect D6a and it simultaneously closes D6c, because teacher-forced perplexity scoring is not obtainable through Ollama by any route.
32. `armada-forge` already carries torch, transformers, and PEFT for `LocalTrainingBackend`, so Requirement 31 adds no dependency.
33. Requirement 31 is chosen over an eval-scoped registration exception because the alternative makes an unpromoted Adapter briefly servable to the whole daemon, which is the exact state Model & Training Pipeline R30 exists to forbid.
34. A limitation is recorded rather than fixed: in-process scoring evaluates the **unquantized** adapter, while what serves is merged, converted to GGUF, and quantized. Quantization can move perplexity by a non-trivial margin, so the gate judges an artifact that is not the shipped artifact. Closing this properly requires scoring the converted GGUF, which inverts the pipeline order. The order is not changed; the limitation is documented.

## Upstream Spec Defects

**Status as of 2026-08-27: D1 through D6 are discharged in their owning specs. Only D7 remains open, deferred by design to Phase 14.** Each defect is corrected in its **owning spec** by re-running `/spec` on that spec. This document corrects none of them in place, because two conflicting statements of the same requirement with nothing declaring precedence is worse than a pointer that says fix it at the source. Each is a blocking precondition on the named phase, not one upfront batch — only D1 blocks Phase 0.

| ID | Owning spec | Defect | Correction | Blocks |
|---|---|---|---|---|
| **D1** ✅ DISCHARGED | model-training-pipeline + platform-overview | The blocked-egress acceptance criterion is incompatible with R4a's acceptance criterion requiring all five shortlist models to answer a chat completion at startup — roughly 10–15 GB of first-boot pulls. | Three named artifacts must change, plus one addition. **(i)** R4a's wording — `"registers one ModelBinding per BaseModel entry with armada-models"` — becomes registering a **record** in `model_bindings`, with materialization a separate explicit act; registering *with armada-models* is what implies the weights are present. **(ii)** R4a's acceptance criterion, specifically its final clause `"and each answers a chat completion request through armada-models"`, which is the single clause forcing 10–15 GB of first-boot pulls; corrected so that every entry has a `promoted` binding row, baked entries carry `materialized: true` and answer a chat completion, and unbaked entries carry `materialized: false`, plus a new criterion asserting first boot transfers zero model bytes. **(iii)** The firewall acceptance criterion is scoped to *after container images are pulled*, or it asserts something no container platform can satisfy. **(iv)** `POST /models/bindings/{tag}/materialize` is named as the mechanism the corrected R4a defers to. | **P0** |
| **D2** ✅ DISCHARGED 2026-08-27 | agent-runtime | The `run_start` event payload is never enumerated. R57 specifies `run_end`'s payload; `run_start` appears only in edge 8. | A requirement specifying `run_start` carries `agent_version_id`, `binding_tag`, `mode`, `workspace_path`, effective budgets, and the fully-qualified tool list after `denied`. | **P3**, **P10** |
| **D3** ✅ DISCHARGED 2026-08-27 | agent-runtime | Two host contracts are unstated or unimplementable. The Docker socket mount is implied rather than required. Edge 7 requires Run start to fail when `workspace_path` does not exist on the host, which a containerized daemon cannot verify by `stat` against a host path it has not mounted. | State the socket mount as a requirement (Requirement 28). Verify `workspace_path` either through a shared workspace root mounted into the daemon, or through Docker rather than the filesystem. | **P5** |
| **D4** ✅ DISCHARGED 2026-08-27 | agent-runtime | R17/R18's liveness check verifies that a pinned `binding_tag` is present and `promoted`. A registered-but-unmaterialized binding passes that check and the Run then blocks behind a multi-gigabyte download. | Extend the liveness check to `materialized`, failing Run start per Requirement 9. | **P7** |
| **D5** ✅ DISCHARGED 2026-08-27 | agent-runtime + team-orchestration | R21 specifies a request `"waits in a FIFO queue for that tag"`; Team Orchestration R32 admits manager requests `"at a higher priority than workers'"`. A priority queue is not FIFO. | Restate as **FIFO within priority class**. | **P8** |
| **D6** ✅ DISCHARGED 2026-08-27 | model-training-pipeline | A cluster of six. **(a)** R34 requires generating from the candidate Adapter via `armada-models`, R30 forbids registering an unpromoted Adapter there, and R31 produces the GGUF only on promotion — a closed loop, present in both eval modes. **(b)** `tool_call_validity` has a 0/0 denominator on supplied-JSONL held-out samples, because no tool schemas are presented at generation time, yet R35 requires candidate ≥ baseline on it. **(c)** `held_out_perplexity` requires teacher-forced scoring of a supplied continuation, which Ollama does not expose by any route. **(d)** R24a caps `max_steps` at 20 and `max_samples` at 200 while leaving `batch_size` and `max_seq_len` uncapped; step cost is roughly linear in their product, so a caller can satisfy every stated cap and still turn a fifteen-minute smoke run into many hours. **(e)** Four acceptance criteria and edge 5 are judge-mode-only but carry no qualifier, and two acceptance criteria give opposite outcomes for `POST /training/runs` against a split-less dataset. **(f)** The edge case list runs 1–21, 25–31, 22, 23, 24. | **(a)** and **(c)**: Requirement 31 — the gate generates and scores in-process; `armada-models` is not involved. **(b)** `tool_call_validity` is null when its denominator is zero and is excluded from R35's comparison, which reduces the default mechanical gate to perplexity alone; R35 must say so rather than leave it emergent. **(d)** Cap `max_seq_len` and `batch_size` in R24a. **(e)** Add judge-mode qualifiers; R33a is the correct reading of the split conflict. **(f)** Renumber. | **P11** |
| **D7** ❌ OPEN — deferred by design | agent-runtime | `egress_allowlist` with `allowed_hosts` is specified as a sandbox profile field, but Docker has no per-container host allowlist primitive. Real implementations are a proxy sidecar on an internal network with outbound DNS blocked, or per-container firewall rules. It is a subsystem, not a config value. | Either specify the proxy subsystem, or defer explicitly with `network: none` as the only supported network mode before Phase 14. | **P14** |
...
### P0 — Foundation
**Depends on:** nothing.
`docker-compose.yml` with all five services, **every migration, `001` through `006`, applied in filename order per Requirement 20a**, service skeletons, healthchecks, the memory limits of Requirement 10, the Ollama environment of Requirement 11, and the asset baking of Requirements 2 and 3.
**Blocked by:** D1.
...
**Depends on:** P0.
Exercises `002_corpora.sql`. The four Source types, extraction, chunking, `bge-small` embedding, per-chunk idempotency on `(content_sha256, source_path)`, the Corpus API, and seed corpora.
...
**Depends on:** P0.
Exercises `model_bindings` from `003_training.sql`; the other four tables that migration creates are exercised at P11. Shortlist validation, R4a base binding registration, R4b reconciliation, `GET /models/bindings`, the `materialized` field of Requirement 7, the capacity enforcement of Requirements 15 and 16, and the backend discriminator of Requirement 19.
...
**Depends on:** P0.
Exercises `005_runs_events.sql`. The plugin registry and the five plugin interfaces, the single-port HTTP and WebSocket listener, transactional gapless `seq`, ordered replay on subscribe, and credential redaction.
**Rationale for position:** invariant 5 lives here and is proven before anything writes to the log.
**Blocked by:** D2.
**Exit:** the process listens on exactly one port serving both `/api/*` and `/ws`; synthetic concurrent appends against a synthetic `runs` row produce gapless `seq`; two subscribers receive identical ordered streams; grepping `events` for a configured credential returns nothing.

### P4 — Agent definition
**Depends on:** P1, P2, P3.
Exercises `004_agents.sql`. The closed schema, error-accumulating validation, reference resolution, the resolver and `resolved_snapshot`, versioning, `refresh-bindings`, the file loader, and both shipped examples.
...
### P8 — Teams
**Depends on:** P7.
Exercises `006_teams.sql`. The team schema and roster resolution, the orchestrator, `delegate` and `list_workers`, child Run creation, tree budgets, synthesis, and scheduler priority.
...
### P9 — Dashboard core
**Depends on:** P4, P7, P8.
The token module, the shell and health strip, the list-plus-drawer pattern, `CorporaPage`, `AgentsPage`, `TeamsPage`, `RunsPage`, ModelsPage's **BaseModel shortlist table only**, both editors, `AgentVersionHistory`, and the run launcher. Includes the additive endpoints whose consumers are in this phase: `DELETE /corpora/{corpus_id}`, `DELETE /api/teams/{team_id}`, the `version` field on run rows, and team-run identification on `GET /api/runs/{run_id}`.

**This phase does not ship `TrainingPage`, ModelsPage's adapter table, `GET /training/runs`, `GET /adapters`, `GET /datasets`, or `GET /api/config/capabilities`.** Those move to P11 under Requirement 21.
**Exit:** the automated token contrast test passes; the desaturation test passes per-family; a fresh installation renders the first-run states the design spec specifies for Corpora, Agents, Teams, Runs, and the BaseModel shortlist.
...
**Depends on:** P1, P2, P9.
Dataset construction, supplied JSONL, the eval split, the `TrainingBackend` interface, `LocalTrainingBackend` with the corrected caps, the in-process evaluation gate of Requirement 31, export and promotion, `TrainingPage`, and the ModelsPage adapter table. Includes the endpoints moved here by Requirement 22: `GET /training/runs`, `GET /adapters`, `GET /datasets`, and `GET /api/config/capabilities`. Extends the forge progress channel to training runs per Requirement 24.
...
- [ ] All six migrations apply cleanly in filename order on a first-ever `docker compose up`, and no phase after Phase 0 introduces a migration.
...
- [ ] No endpoint in the decomposition ships in a phase earlier than its consumer, and no consumer ships earlier than its endpoint.
- [ ] `POST /models/bindings/{tag}/materialize` is callable at the end of Phase 2 with no dashboard present, and materializing `qwen3-1.7b` moves its binding from `materialized: false` to `true`.
- [ ] The forge progress channel emits envelope-conforming messages for a real ingestion at the end of Phase 1, verified by a test client with no dashboard present.
- [ ] Reloading ModelsPage mid-materialization renders the status read from REST, not from a replayed channel message, and the channel replays nothing.
- [ ] A failed materialization renders amber with a retry affordance and never renders `--status-fault`.
...
- `db/migrations/001_init.sql` — new file, pgvector extension, `schema_migrations`, and shared types
- `db/apply-migrations.sh` — new file, applies `db/migrations/*.sql` in filename order, idempotent, skipping already-recorded versions
- `db/migrations/003_training.sql` — new file, five tables including `model_bindings` with `backend`, `materialized`, and `materialization_status`
