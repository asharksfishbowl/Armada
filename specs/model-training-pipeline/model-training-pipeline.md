...
32. `GET /models/bindings` returns every registered ModelBinding with `tag`, `backend`, `base_model_id`, `corpus_name`, `adapter_id` (nullable), `version` (nullable for base bindings), `context_window`, `tool_format`, and `status` (one of `promoted`, `retired`, `missing`). `armada-daemon` reads this endpoint to resolve an Agent's model binding at save time and to verify a pinned binding at Run start.
